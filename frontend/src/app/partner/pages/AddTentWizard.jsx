...
const AddTentWizard = () => {
  ...
  const getStepTitle = () => {
    switch (step) {
      ...
      case 5: return 'Tent Images';
      case 6: return 'Tents & Rooms';
      case 7: return 'Camping Rules';
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <main className="flex-1 w-full max-w-2xl mx-auto p-4 md:p-6 pb-32">
        ...
        <div className="bg-white md:p-6 md:rounded-2xl md:shadow-sm md:border md:border-gray-100 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-gray-500">
                    Tent / Campsite Name <span className="text-red-500">*</span>
                  </label>
                  <input
                    ...
                    onChange={e => {
                      // Strip digits — only letters, spaces, hyphens, apostrophes allowed
                      const lettersOnly = e.target.value.replace(/[0-9]/g, '');
                      updatePropertyForm('propertyName', lettersOnly);
                    }}
                  />
                  {!propertyForm.propertyName && error && (
                    <p className="text-[11px] text-red-500 font-medium mt-1 flex items-center gap-1">
                      <span>⚠</span> Tent / Campsite Name is required
                    </p>
                  )}
                  {propertyForm.propertyName && propertyForm.propertyName.length >= 3 && (
                    <p className="text-[11px] text-green-600 font-medium mt-1 flex items-center gap-1">
                      <span>✓</span> Looks good
                    </p>
                  )}
                  {propertyForm.propertyName && propertyForm.propertyName.length > 0 && propertyForm.propertyName.length < 3 && (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {TENT_TYPES.map(type => (
                      <button
                        ...
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left ${propertyForm.tentType === type.value
                          ? 'border-emerald-500 bg-emerald-50 text-emerald-800 ring-1 ring-emerald-500'
                          : 'border-gray-200 hover:border-emerald-200 hover:bg-emerald-50/50 text-gray-600'
                          }`}
                      >
                        <div className={`p-2 rounded-lg ${propertyForm.tentType === type.value ? 'bg-white text-emerald-600' : 'bg-gray-100 text-gray-500'}`}>
                          <type.icon size={20} />
                        </div>
                ...
                <div className="grid grid-cols-2 gap-4 border-t pt-4">
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-gray-500">Washroom Type</label>
                    <select
                      className="input w-full appearance-none"
                      value={propertyForm.washroomType}
                      onChange={e => updatePropertyForm('washroomType', e.target.value)}
                    >
                      {WASHROOM_TYPES.map(ws => <option key={ws.value} value={ws.value}>{ws.label}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-gray-500">View Type</label>
                    <select
                      className="input w-full appearance-none"
                      value={propertyForm.viewType}
                      onChange={e => updatePropertyForm('viewType', e.target.value)}
                    >
                      {VIEW_TYPES.map(vw => <option key={vw.value} value={vw.value}>{vw.label}</option>)}
                    </select>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-semibold text-gray-500">Camping Activities</label>
            ...
            <div className="space-y-4">
              ...
              {editingRoomType && (
                <div className="bg-white rounded-2xl border border-emerald-100 shadow-lg overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
                  ...
                  <div className="px-4 py-3 bg-emerald-50 border-b border-emerald-100 flex items-center justify-between">
                    <span className="font-bold text-emerald-800 text-sm">
                      {editingRoomTypeIndex === -1 || editingRoomTypeIndex == null ? 'Add Tent Type' : 'Edit Tent Type'}
                    </span>
                  ...
                  {/* All fields — no inner scroll, let page scroll */}
                  <div className="p-4 space-y-4">
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-gray-500">Tent Category Name</label>
                      <input
                        className="input w-full"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <div className="relative">
                          ...
                          <input className="input !pl-10 w-full" type="number" value={editingRoomType.pricePerNight} onChange={e => setEditingRoomType({ ...editingRoomType, pricePerNight: e.target.value.replace(/^0+(?!$)/, '') })} />
                        </div>
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-semibold text-gray-500">Total Units</label>
                        <input className="input w-full" type="number" value={editingRoomType.totalInventory} onChange={e => setEditingRoomType({ ...editingRoomType, totalInventory: e.target.value.replace(/^0+(?!$)/, '') })} />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input className="input w-full" type="number" value={editingRoomType.maxAdults} onChange={e => setEditingRoomType({ ...editingRoomType, maxAdults: e.target.value.replace(/^0+(?!$)/, '') })} />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-semibold text-gray-500">Max Children</label>
                        <input className="input w-full" type="number" value={editingRoomType.maxChildren} onChange={e => setEditingRoomType({ ...editingRoomType, maxChildren: e.target.value.replace(/^0+(?!$)/, '') })} />
                      </div>
                    </div>
          ...
          {step === 7 && (
            <div className="space-y-6">
              {error && <div className="p-3 bg-red-50 text-red-600 text-sm rounded-lg flex items-center gap-2"><span>⚠</span>{error}</div>}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-gray-500">
                      Check-In <span className="text-[10px] font-normal opacity-70">(AM/PM)</span> <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      <input
                        type="time"
                        className={`w-full pl-8 pr-2 py-2 rounded-xl border-2 outline-none text-sm font-medium bg-white focus:border-[#004F4D] focus:ring-2 focus:ring-[#004F4D]/10 transition-all ${propertyForm.checkInTime ? 'text-[#003836]' : 'text-transparent'} ${!propertyForm.checkInTime && error ? 'border-red-400 ring-1 ring-red-400' : 'border-gray-300'}`}
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-gray-500">
                      Check-Out <span className="text-[10px] font-normal opacity-70">(AM/PM)</span> <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      <input
                        type="time"
                        className={`w-full pl-8 pr-2 py-2 rounded-xl border-2 outline-none text-sm font-medium bg-white focus:border-[#004F4D] focus:ring-2 focus:ring-[#004F4D]/10 transition-all ${propertyForm.checkOutTime ? 'text-[#003836]' : 'text-transparent'} ${!propertyForm.checkOutTime && error ? 'border-red-400 ring-1 ring-red-400' : 'border-gray-300'}`}
                ...
                <div className="space-y-1">
                  ...
                  />
                  {propertyForm.cancellationPolicy && propertyForm.cancellationPolicy.length < 10 && (
                    <p className="text-[10px] text-orange-500 mt-0.5">Please describe your policy in more detail</p>
                  )}
                </div>

                <div className="space-y-2 pt-2 border-t border-gray-100">
                  <label className="text-xs font-semibold text-gray-500">Camping Rules</label>
                  <div className="flex flex-wrap gap-2">
          ...
          {step === 9 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Inventory Types ({roomTypes.length})</h3>
                  ...
                  <div className="divide-y divide-gray-50">
                    ...
                    {roomTypes.length === 0 && <div className="text-xs text-red-500 font-medium py-2">No inventory added yet!</div>}
                  </div>
                </div>
          ...
          {step === 10 && (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-gray-500 max-w-sm mx-auto">Your tent property registration has been sent for verification. Our team will review it and get back to you shortly.</p>
              </div>
              <button
