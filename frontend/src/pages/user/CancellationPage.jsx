...
const CancellationPage = () => {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                const res = await legalService.getPage(audience, 'cancellation');
                if (isMounted && res.success) {
                    setPageData(res.page);
                }
            } catch (e) {
                if (isMounted) setError('Unable to load Cancellation Policy.');
            } finally {
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 min-h-[400px]">
                    <div className="flex items-center gap-3 mb-6 text-surface border-b border-gray-100 pb-3">
                        ...
                        <span className="font-bold text-lg">
                            {pageData?.title || 'Cancellation Policy'}
                        </span>
                    </div>
                    ...
                    {loading ? (
                        ...
                        renderContent(pageData?.content || 'Cancellation policy details are being updated.')
                    )}
                </div>
            </div>
        </div>
