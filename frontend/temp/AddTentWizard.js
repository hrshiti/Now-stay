...
const CAMPING_ACTIVITIES = ["Trekking", "Bonfire", "Star Gazing", "Adventure Sports", "Fishing", "Jeep Safari"];
const TENT_TYPES = [
  { value: "luxury", label: "Luxury Tent", icon: Home },
  { value: "safari", label: "Safari Tent", icon: Trees },
  { value: "dome", label: "Dome Tent", icon: Mountain },
  { value: "bell", label: "Bell Tent", icon: Sun },
  { value: "standard", label: "Standard Tent", icon: Home }
];
...
const AddTentWizard = () => {
  ...
  const STORAGE_KEY = `rukko_tent_wizard_draft_${existingProperty?._id || "new"}`;
  useEffect(() => {
      ...
      try {
        ...
        setPropertyForm({
          ...
          documents: REQUIRED_DOCS_TENT.map((d) => {
            const existing = (savedForm.documents || []).find((sd) => sd.type === d.type);
            return { type: d.type, name: d.name, required: d.required, fileUrl: existing?.fileUrl || "" };
          })
        });
        setRoomTypes(savedRooms);
        if (savedProp) setCreatedProperty(savedProp);
      } catch (e) {
        console.error("Failed to load tent draft", e);
      }
    }
  }, []);
  useEffect(() => {
    ...
    const loadForEdit = async () => {
      ...
      try {
        ...
        setPropertyForm({
          ...
          documents: docs.length ? docs.map((d) => ({ type: d.type || d.name, name: d.name, fileUrl: d.fileUrl || "", required: REQUIRED_DOCS_RESORT.find((rd) => rd.type === (d.type || d.name))?.required || false })) : REQUIRED_DOCS_RESORT.map((d) => ({ type: d.type, name: d.name, required: d.required, fileUrl: "" }))
        });
        if (rts.length) {
  ...
  const nextFromBasic = () => {
    setError("");
    if (!propertyForm.propertyName || propertyForm.propertyName.trim().length < 3) {
      setError("Tent / Campsite Name must be at least 3 characters");
      return;
    }
    if (!propertyForm.shortDescription) {
      setError("Short Description is required");
      return;
    }
    if (!propertyForm.tentType) {
  ...
  const nextFromRules = () => {
    ...
    if (!propertyForm.checkOutTime) {
      setError("Check-out time is required");
      return;
    }
    if (!propertyForm.cancellationPolicy || propertyForm.cancellationPolicy.trim().length < 10) {
  ...
  const submitAll = async () => {
    ...
    try {
      ...
      const propertyPayload = {
        ...
        tentType: propertyForm.tentType,
        washroomType: propertyForm.washroomType,
        viewType: propertyForm.viewType,
  ...
  const clearCurrentStep = () => {
    ...
    if (step === 1) {
      setPropertyForm((prev) => ({ ...prev, propertyName: "", description: "", shortDescription: "", tentType: "luxury", washroomType: "attached", viewType: "none", activities: [] }));
    } else if (step === 2) {
      ...
      updatePropertyForm("documents", REQUIRED_DOCS_TENT.map((d) => ({ type: d.type, name: d.name, fileUrl: "" })));
    }
  };
  const handleNext = () => {
    if (loading) return;
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "min-h-screen bg-gray-50 flex flex-col font-sans" }, /* @__PURE__ */ React.createElement("header", { className: "h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 sticky top-0 z-30 shadow-sm" }, /* @__PURE__ */ React.createElement("button", { onClick: handleBack, className: "p-2 -ml-2 text-gray-600 hover:bg-gray-100 rounded-full transition-colors" }, /* @__PURE__ */ React.createElement(ArrowLeft, { size: 20 })), /* @__PURE__ */ React.createElement("div", { className: "text-sm font-bold text-gray-900" }, step <= 9 ? `Step ${step} of 9` : "Registration Complete"), /* @__PURE__ */ React.createElement("button", { onClick: handleExit, className: "p-2 -mr-2 text-gray-600 hover:bg-gray-100 rounded-full transition-colors" }, /* @__PURE__ */ React.createElement(X, { size: 20 }))), /* @__PURE__ */ React.createElement("div", { className: "w-full h-1 bg-gray-200 sticky top-16 z-20" }, /* @__PURE__ */ React.createElement("div", { className: "h-full bg-emerald-600 transition-all duration-500 ease-out", style: { width: `${step / 9 * 100}%` } })), /* @__PURE__ */ React.createElement("main", { className: "flex-1 w-full max-w-2xl mx-auto p-4 md:p-6 pb-32" }, /* @__PURE__ */ React.createElement("div", { className: "mb-6" }, /* @__PURE__ */ React.createElement("h1", { className: "text-2xl font-extrabold text-gray-900 mb-2" }, getStepTitle())), /* @__PURE__ */ React.createElement("div", { className: "bg-white md:p-6 md:rounded-2xl md:shadow-sm md:border md:border-gray-100 space-y-6" }, step === 1 && /* @__PURE__ */ React.createElement("div", { className: "space-y-6" }, error && /* @__PURE__ */ React.createElement("div", { className: "p-3 bg-red-50 text-red-600 text-sm rounded-lg" }, error), /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Tent / Campsite Name ", /* @__PURE__ */ React.createElement("span", { className: "text-red-500" }, "*")), /* @__PURE__ */ React.createElement(
    "input",
    {
      className: `input w-full ${!propertyForm.propertyName && error ? "border-red-400 ring-1 ring-red-400" : ""}`,
      placeholder: "e.g. Desert Safari Camp",
      value: propertyForm.propertyName,
      onChange: (e) => {
        const lettersOnly = e.target.value.replace(/[0-9]/g, "");
        updatePropertyForm("propertyName", lettersOnly);
      }
    }
  ), !propertyForm.propertyName && error && /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-red-500 font-medium mt-1 flex items-center gap-1" }, /* @__PURE__ */ React.createElement("span", null, "\u26A0"), " Tent / Campsite Name is required"), propertyForm.propertyName && propertyForm.propertyName.length >= 3 && /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-green-600 font-medium mt-1 flex items-center gap-1" }, /* @__PURE__ */ React.createElement("span", null, "\u2713"), " Looks good"), propertyForm.propertyName && propertyForm.propertyName.length > 0 && propertyForm.propertyName.length < 3 && /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-orange-500 font-medium mt-1" }, "Name must be at least 3 characters")), /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Tent Type"), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-3" }, TENT_TYPES.map((type) => /* @__PURE__ */ React.createElement(
    "button",
    {
      key: type.value,
      type: "button",
      onClick: () => updatePropertyForm("tentType", type.value),
      className: `flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left ${propertyForm.tentType === type.value ? "border-emerald-500 bg-emerald-50 text-emerald-800 ring-1 ring-emerald-500" : "border-gray-200 hover:border-emerald-200 hover:bg-emerald-50/50 text-gray-600"}`
    },
    /* @__PURE__ */ React.createElement("div", { className: `p-2 rounded-lg ${propertyForm.tentType === type.value ? "bg-white text-emerald-600" : "bg-gray-100 text-gray-500"}` }, /* @__PURE__ */ React.createElement(type.icon, { size: 20 })),
    /* @__PURE__ */ React.createElement("span", { className: "text-sm font-bold" }, type.label)
  )))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-4 border-t pt-4" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Washroom Type"), /* @__PURE__ */ React.createElement(
    "select",
    {
      className: "input w-full appearance-none",
      value: propertyForm.washroomType,
      onChange: (e) => updatePropertyForm("washroomType", e.target.value)
    },
    WASHROOM_TYPES.map((ws) => /* @__PURE__ */ React.createElement("option", { key: ws.value, value: ws.value }, ws.label))
  )), /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "View Type"), /* @__PURE__ */ React.createElement(
    "select",
    {
      className: "input w-full appearance-none",
      value: propertyForm.viewType,
      onChange: (e) => updatePropertyForm("viewType", e.target.value)
    },
    VIEW_TYPES.map((vw) => /* @__PURE__ */ React.createElement("option", { key: vw.value, value: vw.value }, vw.label))
  ))), /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Camping Activities"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, CAMPING_ACTIVITIES.map((act) => /* @__PURE__ */ React.createElement(
    "button",
    {
      key: act,
      type: "button",
      ...
      className: `px-4 py-2 rounded-xl text-xs font-bold transition-all border ${propertyForm.activities.includes(act) ? "bg-emerald-600 text-white border-emerald-600 shadow-md transform scale-[1.02]" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`
    },
    act
  )))), /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Description"), /* @__PURE__ */ React.createElement("textarea", { className: "input w-full h-24", placeholder: "Brief summary for listings...", value: propertyForm.shortDescription, onChange: (e) => updatePropertyForm("shortDescription", e.target.value) })), /* @__PURE__ */ React.createElement("div", { className: "hidden" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Detailed Description"), /* @__PURE__ */ React.createElement("textarea", { className: "input w-full min-h-[100px]", placeholder: "Tell guests what makes your resort unique...", value: propertyForm.description, onChange: (e) => updatePropertyForm("description", e.target.value) })), /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Contact Number (For Guest Inquiries)"), /* @__PURE__ */ React.createElement(
    "input",
    {
      type: "tel",
  ...
  )))), step === 2 && /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500 mb-1 block" }, "Search Address"), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2" }, /* @__PURE__ */ React.createElement(
    "input",
    {
      className: "input w-full",
    ...
    "Search"
  )), locationResults.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "border border-gray-200 rounded-xl overflow-hidden mt-1 shadow-lg bg-white max-h-48 overflow-y-auto z-10 relative" }, locationResults.map((p, i) => /* @__PURE__ */ React.createElement(
    "button",
    {
      key: i,
      type: "button",
    ...
    /* @__PURE__ */ React.createElement("div", { className: "font-medium text-gray-900" }, p.name),
    /* @__PURE__ */ React.createElement("div", { className: "text-xs text-gray-500" }, p.formatted_address)
  )))), /* @__PURE__ */ React.createElement("div", { className: "relative py-2" }, /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 flex items-center" }, /* @__PURE__ */ React.createElement("div", { className: "w-full border-t border-gray-200" })), /* @__PURE__ */ React.createElement("div", { className: "relative flex justify-center text-xs uppercase" }, /* @__PURE__ */ React.createElement("span", { className: "bg-white px-2 text-gray-400 font-medium" }, "Or Enter Manually"))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-3" }, /* @__PURE__ */ React.createElement("input", { className: "input col-span-2", placeholder: "Full Address", value: propertyForm.address.fullAddress, onChange: (e) => updatePropertyForm(["address", "fullAddress"], e.target.value) }), /* @__PURE__ */ React.createElement("input", { className: "input", placeholder: "City", value: propertyForm.address.city, onChange: (e) => updatePropertyForm(["address", "city"], e.target.value) }), /* @__PURE__ */ React.createElement("input", { className: "input", placeholder: "State", value: propertyForm.address.state, onChange: (e) => updatePropertyForm(["address", "state"], e.target.value) }), /* @__PURE__ */ React.createElement("input", { className: "input", placeholder: "Country", value: propertyForm.address.country, onChange: (e) => updatePropertyForm(["address", "country"], e.target.value) }), /* @__PURE__ */ React.createElement("input", { className: "input", placeholder: "Pincode", maxLength: 6, value: propertyForm.address.pincode, onChange: (e) => updatePropertyForm(["address", "pincode"], e.target.value.replace(/\D/g, "")) }), /* @__PURE__ */ React.createElement("input", { className: "input", placeholder: "Area", value: propertyForm.address.area, onChange: (e) => updatePropertyForm(["address", "area"], e.target.value) })), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
  ...
  )), step === 3 && /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 sm:grid-cols-3 gap-3" }, CAMPING_AMENITIES.map((am) => {
    ...
    return /* @__PURE__ */ React.createElement(
      "button",
      {
        ...
        className: `
                          ...
                          ${isSelected ? "bg-emerald-600 border-emerald-600 text-white shadow-md transform scale-[1.02]" : "bg-white border-gray-200 text-gray-600 hover:border-emerald-200 hover:bg-emerald-50/30"}
                        `
      },
      /* @__PURE__ */ React.createElement("span", { className: "font-semibold text-sm" }, am),
      isSelected && /* @__PURE__ */ React.createElement("div", { className: "absolute top-2 right-2 text-white/80" }, /* @__PURE__ */ React.createElement(CheckCircle, { size: 14 }))
    );
    ...
    propertyForm.coverImage ? /* @__PURE__ */ React.createElement("img", { src: propertyForm.coverImage, alt: "Cover", className: "w-full h-full object-cover" }) : /* @__PURE__ */ React.createElement("div", { className: "flex flex-col items-center text-gray-400 group-hover:text-emerald-600 transition-colors" }, /* @__PURE__ */ React.createElement(Image, { size: 40, className: "mb-2 opacity-50" }), /* @__PURE__ */ React.createElement("span", { className: "text-xs font-bold" }, isFlutter ? "Take/Upload Cover Photo" : "Upload Cover Photo")),
    uploading === "cover" && /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 bg-white/80 flex flex-col gap-2 items-center justify-center" }, /* @__PURE__ */ React.createElement(Loader2, { className: "animate-spin text-emerald-600", size: 32 }), /* @__PURE__ */ React.createElement("span", { className: "text-sm font-bold text-emerald-700" }, "Uploading...")),
    propertyForm.coverImage && /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        onClick: (e) => {
  ...
  ), /* @__PURE__ */ React.createElement("input", { ref: coverImageFileInputRef, type: "file", accept: "image/*", className: "hidden", onChange: (e) => handleImageUpload(e, "cover") })), /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-center" }, /* @__PURE__ */ React.createElement("label", { className: "text-sm font-bold text-gray-900" }, "Property Gallery"), /* @__PURE__ */ React.createElement("span", { className: "text-xs text-gray-500" }, propertyForm.propertyImages.length, " images")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-3 sm:grid-cols-4 gap-3" }, propertyForm.propertyImages.map((img, i) => /* @__PURE__ */ React.createElement("div", { key: i, className: "relative aspect-square rounded-xl overflow-hidden border border-gray-200 group" }, /* @__PURE__ */ React.createElement("img", { src: img, alt: `Gallery ${i}`, className: "w-full h-full object-cover" }), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
  ...
  )), /* @__PURE__ */ React.createElement("input", { ref: propertyImagesFileInputRef, type: "file", multiple: true, accept: "image/*", className: "hidden", onChange: (e) => handleImageUpload(e, "gallery") })))), step === 6 && /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, error && /* @__PURE__ */ React.createElement("div", { className: "p-3 bg-red-50 text-red-600 text-sm rounded-lg" }, error), !editingRoomType && /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, roomTypes.length === 0 ? /* @__PURE__ */ React.createElement("div", { className: "text-center py-10 px-6 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50" }, /* @__PURE__ */ React.createElement("div", { className: "w-12 h-12 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center mx-auto mb-3" }, /* @__PURE__ */ React.createElement(BedDouble, { size: 24 })), /* @__PURE__ */ React.createElement("p", { className: "text-gray-500 font-medium" }, "No cottages or rooms added yet"), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-gray-400 mt-1" }, "Add details for atleast one category.")) : /* @__PURE__ */ React.createElement("div", { className: "grid gap-4" }, roomTypes.map((rt, index) => /* @__PURE__ */ React.createElement("div", { key: rt.id || index, className: "p-4 bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-shadow relative overflow-hidden" }, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-start mb-2" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { className: "font-bold text-gray-900" }, rt.name), /* @__PURE__ */ React.createElement("div", { className: "text-xs text-gray-500 font-medium mt-0.5" }, "Inventory: ", /* @__PURE__ */ React.createElement("span", { className: "text-gray-900" }, rt.totalInventory), " \xB7 Capacity: ", /* @__PURE__ */ React.createElement("span", { className: "text-gray-900" }, rt.maxAdults, "A, ", rt.maxChildren, "C"))), /* @__PURE__ */ React.createElement("div", { className: "text-lg font-bold text-emerald-600" }, "\u20B9 ", rt.pricePerNight)), rt.amenities && rt.amenities.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1 mb-3" }, rt.amenities.slice(0, 3).map((a) => /* @__PURE__ */ React.createElement("span", { key: a, className: "px-2 py-0.5 rounded-md bg-gray-100 text-gray-600 text-[10px] font-medium border border-gray-200" }, a)), rt.amenities.length > 3 && /* @__PURE__ */ React.createElement("span", { className: "px-2 py-0.5 text-[10px] text-gray-400" }, "+", rt.amenities.length - 3, " more")), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 mt-2 pt-3 border-t border-gray-100" }, /* @__PURE__ */ React.createElement("button", { onClick: () => startEditRoomType(index), className: "flex-1 py-2 text-xs font-bold text-emerald-700 bg-emerald-50 rounded-lg hover:bg-emerald-100 transition-colors" }, "Edit"), /* @__PURE__ */ React.createElement("button", { onClick: () => deleteRoomType(index), className: "px-3 py-2 text-xs font-bold text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors" }, /* @__PURE__ */ React.createElement(Trash2, { size: 16 })))))), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
  ...
  )), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Price / Night (\u20B9)"), /* @__PURE__ */ React.createElement("div", { className: "relative" }, /* @__PURE__ */ React.createElement("span", { className: "absolute left-[14px] top-1/2 -translate-y-1/2 text-gray-400 font-bold" }, "\u20B9"), /* @__PURE__ */ React.createElement("input", { className: "input !pl-10 w-full", type: "number", value: editingRoomType.pricePerNight, onChange: (e) => setEditingRoomType({ ...editingRoomType, pricePerNight: e.target.value.replace(/^0+(?!$)/, "") }) }))), /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Total Units"), /* @__PURE__ */ React.createElement("input", { className: "input w-full", type: "number", value: editingRoomType.totalInventory, onChange: (e) => setEditingRoomType({ ...editingRoomType, totalInventory: e.target.value.replace(/^0+(?!$)/, "") }) }))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Max Adults"), /* @__PURE__ */ React.createElement("input", { className: "input w-full", type: "number", value: editingRoomType.maxAdults, onChange: (e) => setEditingRoomType({ ...editingRoomType, maxAdults: e.target.value.replace(/^0+(?!$)/, "") }) })), /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Max Children"), /* @__PURE__ */ React.createElement("input", { className: "input w-full", type: "number", value: editingRoomType.maxChildren, onChange: (e) => setEditingRoomType({ ...editingRoomType, maxChildren: e.target.value.replace(/^0+(?!$)/, "") }) }))), /* @__PURE__ */ React.createElement("div", { className: "bg-emerald-50/50 p-3 rounded-xl border border-emerald-100 space-y-3" }, /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-bold text-emerald-800 uppercase tracking-wider" }, "Pricing Configuration"), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Base Adults Included"), /* @__PURE__ */ React.createElement(
    "input",
    {
  ...
  ), Number(editingRoomType.baseChildren) > Number(editingRoomType.maxChildren) && /* @__PURE__ */ React.createElement("p", { className: "text-[10px] text-red-500 font-bold animate-pulse mt-0.5" }, "Exceeds Max Children (", editingRoomType.maxChildren, ")"))), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Extra Adult Price (\u20B9)"), /* @__PURE__ */ React.createElement("div", { className: "relative" }, /* @__PURE__ */ React.createElement("span", { className: "absolute left-[14px] top-1/2 -translate-y-1/2 text-gray-400 font-bold" }, "\u20B9"), /* @__PURE__ */ React.createElement("input", { className: "input !pl-10 w-full bg-white", type: "number", value: editingRoomType.extraAdultPrice, onChange: (e) => setEditingRoomType({ ...editingRoomType, extraAdultPrice: e.target.value.replace(/^0+(?!$)/, "") }) }))), /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Extra Child Price (\u20B9)"), /* @__PURE__ */ React.createElement("div", { className: "relative" }, /* @__PURE__ */ React.createElement("span", { className: "absolute left-[14px] top-1/2 -translate-y-1/2 text-gray-400 font-bold" }, "\u20B9"), /* @__PURE__ */ React.createElement("input", { className: "input !pl-10 w-full bg-white", type: "number", value: editingRoomType.extraChildPrice, onChange: (e) => setEditingRoomType({ ...editingRoomType, extraChildPrice: e.target.value.replace(/^0+(?!$)/, "") }) }))))), /* @__PURE__ */ React.createElement("div", { className: "space-y-2 pt-2 border-t border-gray-100" }, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-center" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Photos"), /* @__PURE__ */ React.createElement("span", { className: "text-[10px] text-gray-400" }, (editingRoomType.images || []).filter(Boolean).length, " / 3 min")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, (editingRoomType.images || []).filter(Boolean).map((img, i) => /* @__PURE__ */ React.createElement("div", { key: i, className: "relative w-16 h-16 rounded-lg overflow-hidden border border-gray-200 group" }, /* @__PURE__ */ React.createElement("img", { src: img, alt: "", className: "w-full h-full object-cover" }), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => handleRemoveImage(img, "room", i), className: "absolute top-1 right-1 w-5 h-5 rounded-full bg-white text-red-500 flex items-center justify-center shadow-sm opacity-0 group-hover:opacity-100 transition-opacity" }, /* @__PURE__ */ React.createElement(X, { size: 12 })))), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => isFlutter ? handleCameraUpload("room", (url) => setEditingRoomType((prev) => ({ ...prev, images: [...prev.images || [], url] }))) : roomImagesFileInputRef.current?.click(), disabled: !!uploading, className: "w-16 h-16 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 hover:text-emerald-600 hover:border-emerald-400 hover:bg-emerald-50 transition-all" }, uploading === "room" ? /* @__PURE__ */ React.createElement(Loader2, { size: 20, className: "animate-spin text-emerald-600" }) : /* @__PURE__ */ React.createElement(Plus, { size: 20 })), /* @__PURE__ */ React.createElement("input", { ref: roomImagesFileInputRef, type: "file", multiple: true, accept: "image/*", className: "hidden", onChange: (e) => {
  ...
  } }))), /* @__PURE__ */ React.createElement("div", { className: "space-y-2 pt-2 border-t border-gray-100" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Amenities"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, ROOM_AMENITIES_OPTIONS.map((opt) => {
  ...
  }))), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 pt-2" }, /* @__PURE__ */ React.createElement("button", { type: "button", onClick: cancelEditRoomType, className: "flex-1 py-3 text-gray-600 font-semibold bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors" }, "Cancel"), /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      onClick: () => {
    ...
    "Clear"
  ), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: saveRoomType, className: "flex-1 py-3 text-white font-bold bg-emerald-600 rounded-xl hover:bg-emerald-700 shadow-md shadow-emerald-100 transition-all transform active:scale-95" }, "Save"))))), step === 7 && /* @__PURE__ */ React.createElement("div", { className: "space-y-6" }, error && /* @__PURE__ */ React.createElement("div", { className: "p-3 bg-red-50 text-red-600 text-sm rounded-lg flex items-center gap-2" }, /* @__PURE__ */ React.createElement("span", null, "\u26A0"), error), /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Check-In Time ", /* @__PURE__ */ React.createElement("span", { className: "text-red-500" }, "*")), /* @__PURE__ */ React.createElement("div", { className: "relative" }, /* @__PURE__ */ React.createElement(
    "input",
    {
      type: "time",
    ...
    "textarea",
    {
      className: `input w-full min-h-[100px] resize-none ${!propertyForm.cancellationPolicy && error ? "border-red-400 ring-1 ring-red-400" : ""}`,
      placeholder: "e.g., Free cancellation before 24 hours of check-in...",
      value: propertyForm.cancellationPolicy,
      onChange: (e) => updatePropertyForm("cancellationPolicy", e.target.value)
    }
  ), propertyForm.cancellationPolicy && propertyForm.cancellationPolicy.length < 10 && /* @__PURE__ */ React.createElement("p", { className: "text-[10px] text-orange-500 mt-0.5" }, "Please describe your policy in more detail")), /* @__PURE__ */ React.createElement("div", { className: "space-y-2 pt-2 border-t border-gray-100" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-semibold text-gray-500" }, "Camping Rules"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, HOUSE_RULES_OPTIONS.map((r) => {
    ...
    "Go to My Properties"
  )))), step < 10 && /* @__PURE__ */ React.createElement("footer", { className: "fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 md:px-6 z-40 bg-white/80 backdrop-blur-md" }, /* @__PURE__ */ React.createElement("div", { className: "max-w-2xl mx-auto flex items-center justify-between gap-4" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      onClick: handleBack,
    ...
    loading && /* @__PURE__ */ React.createElement("div", { className: "w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" }),
